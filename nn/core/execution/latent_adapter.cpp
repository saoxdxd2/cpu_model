...
LatentAdapter::LatentAdapter(size_t in_dim) : in_dim_(in_dim) {
    weights_.reserve(nca::config::D_MODEL);
    for(size_t i=0; i<nca::config::D_MODEL; ++i) {
        weights_.emplace_back(in_dim_ / 32);
    }

    std::mt19937 gen(42);
    std::uniform_real_distribution<float> dist(-0.02f, 0.02f);
    
    auto temp = nca::simd::make_aligned_unique<float[]>(in_dim_);
    for(size_t i=0; i<nca::config::D_MODEL; ++i) {
        for(size_t j=0; j<in_dim_; ++j) temp[j] = dist(gen);
        nca::linalg::mx_quantize_w(temp.get(), weights_[i]);
    }
}

void LatentAdapter::project(const float* in, float* out) {
    nca::linalg::MXUINT8Tensor in_q(in_dim_ / 32);
